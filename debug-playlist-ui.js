(() => {
  ...
  if (!directory || !viewTabs || !favoritesToggle) return;
  ...
  function ensureTabs() {
    if (!viewTabs.querySelector('[data-view="favorites"]')) {
      const favoritesTab = document.createElement('button');
      favoritesTab.className = 'view-tab';
      favoritesTab.dataset.view = 'favorites';
      favoritesTab.type = 'button';
      favoritesTab.setAttribute('role', 'tab');
      favoritesTab.setAttribute('aria-selected', 'false');
      favoritesTab.textContent = '收藏';
      viewTabs.appendChild(favoritesTab);
    }
    if (!viewTabs.querySelector('[data-view="playlist"]')) {
      const playlistTab = document.createElement('button');
      playlistTab.className = 'view-tab';
      playlistTab.dataset.view = 'playlist';
      playlistTab.type = 'button';
      playlistTab.setAttribute('role', 'tab');
      playlistTab.setAttribute('aria-selected', 'false');
      playlistTab.textContent = '播放列表';
      viewTabs.appendChild(playlistTab);
    }
  ...
  style.textContent = `
    ...
    .debug-playlist-toolbar>div{display:flex;align-items:baseline;gap:8px}.debug-playlist-toolbar span{font-size:11px;color:var(--muted)}
    ...
    @media(max-width:560px){.playlist-add-card{width:30px;height:30px;font-size:18px}.debug-playlist-row{grid-template-columns:24px minmax(0,1fr)}.debug-playlist-controls{grid-column:2;justify-content:flex-start}}
  `;
