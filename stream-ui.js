      ...
      <section class="stream-view" data-stream-view="1">
        ...
        <div class="stream-rows">
          ${queue.map((item, index) => `
            <article class="stream-row" data-queue-key="${esc(item.key)}" data-show-id="${esc(item.showId)}" data-episode-id="${esc(item.episodeId)}">
              <div class="stream-index">${index + 1}</div>
              <div class="stream-copy">
                <div class="show-name">${esc(item.showName)}</div>
                <div class="episode-title" data-stream-episode-link role="button" tabindex="0" title="打开这个单集">${esc(item.title)}</div>
              </div>
